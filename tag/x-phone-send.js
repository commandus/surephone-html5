...
function hasToken() {
	return window.localStorage.getItem('psentFirebaseMessagingToken') != null;
}

function getToken() {
	return window.localStorage.getItem('psentFirebaseMessagingToken');
}

function getUserid() {
	return window.localStorage.getItem('puserid');
}

function getCreated() {
	return window.localStorage.getItem('pcreated');
}

function getAccesskey() {
	return window.localStorage.getItem('paccesskey');
}

function isTokenSaved(currentToken) {
    return window.localStorage.getItem('psentFirebaseMessagingToken') == currentToken;
}
...
/**
  ...
  */
function saveUser(self, token) {
	window.localStorage.setItem('psentFirebaseMessagingToken', token);
	var config = {
		parent_id: self.mPid,
		instance: token,
		name: self.mLabel,
	...
	rpcPost(self.mAPI + '/addusrdev', config, function(v) {
		if (v.length > 20) {
			window.localStorage.setItem('puserid', v[1]);
			self.mInfoPid.textContent = 'PID: ' + v[1] 
				+ ' userid=' + v[1]
				+ '&created=' + v[11]
				+ '&accesskey=' + v[20];
			window.localStorage.setItem('pcreated', v[11]);
			window.localStorage.setItem('paccesskey', v[20]);
		}
	} );
}

function removeUser(apiUri, userid, created, accesskey, token) {
	window.localStorage.removeItem('psentFirebaseMessagingToken');
	var config = {
		userid: window.localStorage.getItem('puserid'),
		created: window.localStorage.getItem('pcreated'),
		accesskey: window.localStorage.getItem('paccesskey')
	};
	rpcPost(apiUri + '/rmusr', config, function(v) {
		window.localStorage.removeItem('puserid');
		window.localStorage.removeItem('pcreated');
		window.localStorage.removeItem('paccesskey');
		} );
}
	...
	self.mMessaging.getToken()
		.then(function(currentToken) {
			self.mMessaging.deleteToken(currentToken)
				.then(function() {
					self.mInfoPid.textContent = 'нет PID';
...
function sendMsg(self)
{
	var config = {
		...
		accesskey: window.localStorage.getItem('paccesskey'),
		key: self.mNumber,
		title: self.mSubject.value,
		text: self.mTextArea.value,
		icon: self.mIcon.value,
		click_action: self.mUrl.value
	};
	self.mValue = self.mInput.value;
	self.mNumber = self.parse(self.mValue);

	console.log(self.mInput.value);
	console.log(self.mNumber);
	
	rpcPost(self.mAPI + '/spk', config, function(v) {
		self.progress(false);
		} );
}
...
class PhoneNumberSend extends HTMLElement {
	constructor() {
		super();
	...
	initDOM(t) {
		...
		t.mFormRow2 = document.createElement("div"); 
		t.mFormRow2.setAttribute('class','row');

		t.mFormRow3 = document.createElement("div"); 
		t.mFormRow3.setAttribute('class','row');

		t.mInfo = document.createElement("div"); 
		t.mInfo.setAttribute('id','info');

		t.mField1 = document.createElement("div"); 
		t.mField1.setAttribute('class','input-field col s8');
		...
		t.mField2.setAttribute('class','input-field col s4');
		...
		t.mSubmit.textContent = 'Отправить';	
		t.mSubmit.addEventListener('click', e => {
			t.sendMessage();
		});

		t.mSubject = document.createElement("input"); 
		t.mSubject.setAttribute('id','subj');
		t.mSubject.setAttribute('name','subj');
		t.mSubject.setAttribute('type','text');
		t.mSubject.setAttribute('class','validate col s8');
		t.mSubject.setAttribute('value','Новое!');

		t.mTextArea = document.createElement("textarea"); 
		t.mTextArea.setAttribute('class','materialize-textarea col s8');

		t.mIcon = document.createElement("input"); 
		t.mIcon.setAttribute('id','icon');
		t.mIcon.setAttribute('name','icon');
		t.mIcon.setAttribute('type','text');
		t.mIcon.setAttribute('class','validate col s8');
		t.mIcon.setAttribute('value','https://surephone.commandus.com/i/72.png');

		t.mUrl = document.createElement("input"); 
		t.mUrl.setAttribute('id','url');
		t.mUrl.setAttribute('name','url');
		t.mUrl.setAttribute('type','text');
		t.mUrl.setAttribute('class','validate col s8');
		t.mUrl.setAttribute('value','https://surephone.commandus.com/incoming');

		t.mButtonSubscribe = document.createElement("button"); 
		t.mButtonSubscribe.setAttribute('class','btn waves-effect waves-light col s4');
		t.mButtonSubscribe.addEventListener('click', e => {
			t.subscribe();
			// Materialize.toast(this.mToastDelete, 10000);
		});
		t.mButtonSubscribe.textContent = 'Включить отправку';	

		t.mButtonUnsubscribe = document.createElement("button"); 
		t.mButtonUnsubscribe.setAttribute('class','btn waves-effect red accent-1 col s4');
		t.mButtonUnsubscribe.addEventListener('click', e => {
			this.unsubscribe();
			// Materialize.toast(this.mToastDelete, 10000);
		});
		t.mButtonUnsubscribe.textContent = 'Прекратить все';	

		t.mInfoPid = document.createElement("div"); 
		t.mInfoPid.setAttribute('class','col s4');
		var pid = getUserid();

		if (pid) {
			t.mInfoPid.textContent = 'PID: ' + pid 
			+ ' userid=' + pid
			+ '&created=' + getCreated()
			+ '&accesskey=' + getAccesskey();
		} else {
			t.mInfoPid.textContent = 'нет PID';	
		}

		t.mToastDelete = document.createElement("div"); 
		t.mToastDeleteMsg = document.createElement("span"); 
		t.mToastDeleteMsg.textContent = "Подтвердите удаление";
		t.mToastDeleteButton = document.createElement("button"); 
		t.mToastDeleteButton.setAttribute('class','btn waves-effect waves-light');
		t.mToastDeleteButton.textContent = 'Удалить';	
		t.mToastDeleteButton.addEventListener('click', e => {
			t.unsubscribe();
		});
		t.mToastDelete.appendChild(t.mToastDeleteMsg);
		t.mToastDelete.appendChild(t.mToastDeleteButton);
		...
		t.mForm.appendChild(t.mFormRow2);
		t.mForm.appendChild(t.mFormRow3);
		...
		t.mFormRow2.appendChild(t.mSubject);
		t.mFormRow2.appendChild(t.mTextArea);
		t.mFormRow2.appendChild(t.mIcon);
		t.mFormRow2.appendChild(t.mUrl);
		t.mFormRow3.appendChild(t.mButtonUnsubscribe);
		t.mFormRow3.appendChild(t.mInfoPid);
		t.mFormRow3.appendChild(t.mButtonSubscribe);
	...
	setButton() 
	{
		var s;
		if (hasToken()) {
			this.mButtonSubscribe.setAttribute('disabled', '');
			this.mSubmit.removeAttribute('disabled');
			this.mInput.removeAttribute('disabled');
			this.mButtonUnsubscribe.removeAttribute('disabled');
			this.mTextArea.removeAttribute('disabled');
		}
		else {
			this.mButtonSubscribe.removeAttribute('disabled');
			this.mButtonUnsubscribe.setAttribute('disabled', '');
			this.mSubmit.setAttribute('disabled', '');
			this.mInput.setAttribute('disabled', '');
			this.mTextArea.setAttribute('disabled', '');
		}
		this.fireEvent();
	}
	...
	fireEvent() {
		if (this.mOntoken) {
			window.console && console.log(this.mOntoken);
...
customElements.define('x-phone-send', PhoneNumberSend);